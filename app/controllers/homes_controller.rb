class HomesController < ApplicationController
  ...
  def youtube_result
    ...
    @youtube = Youtube.where(live: "none").order(publishedAt: :desc).to_json(:except => [:id]).limit(500)
  ...
  def youtube_ytn
    @youtube = Youtube.where(channelTitle: "UChlgI3UHCOnwUGzWzbJ3H5w", live: 'none').order(publishedAt: :desc).to_json(:except => [:id]).limit(400)
